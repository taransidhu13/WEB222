...
function speciesCoordinates3(data) {
  ...
  for (const observation of results) {
    const species = observation.species_guess;
    const location = observation.location;
    console.log(`\"${species}\" observed at coordinates (${location[0]}, ${location[1]})`);
  }
}
...
function observationsByQualityGrade(data, qualityGrade) {
  if (
    ...
    (typeof qualityGrade !== 'string' ||
      !['research', 'needs_id', 'casual'].includes(qualityGrade.toLowerCase()))
  ) {
  ...
  for (const observation of data.results) {
    const observationQualityGrade = observation.quality_grade ? observation.quality_grade : null;
    if (qualityGrade === null && observationQualityGrade === null) {
      matchingObservations.push(observation);
    } else if (qualityGrade !== null && observationQualityGrade === qualityGrade.toLowerCase()) {
      matchingObservations.push(observation);
    }
  }
...
function observationsByQualityGrades(data, ...qualityGrades) {
  ...
  for (const qualityGrade of qualityGrades) {
    const observations = observationsByQualityGrade(data, qualityGrade);
...
function getObservationsByTaxa(data, ...taxaNames) {
  ...
  taxaNames.forEach((taxonName) => {
    if (validTaxa.includes(taxonName)) {
...
function getObservationsByLocation(data, options = {}) {
  return data.results.filter((observation) => {
    const lat = parseFloat(observation.location.split(',')[0]);
    const lng = parseFloat(observation.location.split(',')[1]);

    if (!options.lat && !options.lng) {
      return true;
    }

    if (options.lat === lat && options.lng === lng) {
      return lat, lng;
    } else if (options.lat.min <= lat && lat <= options.lat.max) {
      return true;
    }

    if (options.lng.min <= lng && lng <= options.lng.max) {
      return true;
    }
...
function getPlaceURLs(data) {
  ...
  data.results.forEach((observation) => {
    if (observation.place_ids && Array.isArray(observation.place_ids)) {
